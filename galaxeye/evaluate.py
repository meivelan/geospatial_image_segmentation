...
def visualize_predictions(model, dataset, num_samples=5):
    ...
    with torch.no_grad():
        for idx in range(len(dataset)):
            ...
            with rasterio.open(
                os.path.join(dataset.root_dir, dataset.split, "pre-event", fname)
            ) as src:
                original_eo = src.read().astype(np.float32) / 255.0
            original_eo_display = original_eo.transpose(1, 2, 0)[:, :, :3]

            with rasterio.open(
                os.path.join(dataset.root_dir, dataset.split, "post-event", fname)
            ) as src:
            ...
            input_image_for_display = (
                transformed_image_tensor.cpu().numpy().transpose(1, 2, 0)
            )
...
def evaluate_all_runs(runs, dataset_root, test_loader, test_dataset, norm_stats):
    """
    ...
    for i in runs:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ckpt = torch.load(
            os.path.join(dataset_root, i, "best.pth"), map_location=device
        )
