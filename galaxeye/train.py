...
from .model import get_model, combined_loss, compute_metrics, compute_confusion_matrix
...
def save_checkpoint(epoch, model, optimizer, scaler, metrics, best_f1, is_best=False):
...
def train_epoch(model, loader, optimizer, scaler, scheduler, epoch):
    ...
    for batch_idx, (images, masks) in enumerate(loader):
        ...
        if batch_idx % 20 == 0:
            print(
                f"  [{batch_idx:>4}/{len(loader)}] " f"loss={loss.item():.4f}", end="\r"
            )
...
def validate(model, loader, epoch):
    ...
    cm = compute_confusion_matrix(all_preds, all_targets, CONFIG["threshold"])
...
def train(train_loader, val_loader):
    ...
    scaler = torch.amp.GradScaler(device="cuda", enabled=not CONFIG["use_bf16"])
    ...
    for epoch in range(start_epoch, CONFIG["max_epochs"]):
        ...
        tr_loss = train_epoch(model, train_loader, optimizer, scaler, scheduler, epoch)
        ...
        print(
            f"{epoch:>6} {tr_loss:>8.4f} {va_loss:>8.4f} "
        ...
        if is_best:
            best_f1 = metrics["f1"]
        save_checkpoint(epoch, model, optimizer, scaler, metrics, best_f1, is_best)
